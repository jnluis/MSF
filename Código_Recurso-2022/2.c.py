...
def magnus_3D(r0,v0,a0,rot,p_ar,r,n,dt,m):
    ...
    for i in range(n):
        if vv[i] < 30:
            cres = 0.5
            vv[i]=np.sqrt(vx[i]**2 +vy[i]**2 +vz[i]**2)
            ...
            ax[i]=a0[0]-(cres/(2*m))*A*dar*abs(vv[i])*vx[i]+mag_x
            ay[i]=a0[1]-(cres/(2*m))*A*dar*abs(vv[i])*vy[i]+mag_y
            az[i]=a0[2]-(cres/(2*m))*A*dar*abs(vv[i])*vz[i]+mag_z
